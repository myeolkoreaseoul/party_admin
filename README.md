...
### 🔴 미해결 문제 (다음 작업 시 우선 확인)

**이상윤 고객 정보 연동 문제**
- **증상**: 고객관리에서 수기 입력한 이상윤의 정보(이름, 성별, 나이, 키, 직업)가 예약관리/라인업에 표시 안 됨
- **현재 시도한 것**:
  - 전화번호 매칭 → 실패 (전화번호 불일치 추정)
  - 이름(crewName) 매칭 fallback 추가 → 아직 작동 안 함
- **확인 필요**:
  1. 예약의 `crewName`과 고객관리의 `name`이 정확히 일치하는지
  2. 콘솔(F12)에서 `[예약관리] 이상윤: customer=true/false` 확인
  3. 고객관리에 저장된 이상윤의 전화번호와 예약의 전화번호 비교
- **근본 해결책**: 고객 데이터 연결 방식 재설계 필요 (전화번호 의존 → ID 기반?)
