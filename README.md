...
**⚠️ 로컬 파일 직접 수정 금지! 반드시 아래 규칙 따를 것:**

1. **파일 읽기**: 로컬 말고 **git에서 최신 pull** 후 읽기
2. **파일 수정**: 수정 후 반드시 **git commit + push**
3. **GAS 수정**: `GAS_SMS_SENDER.js` 수정 후 **clasp push**로 GAS 반영
...
**이유**: 로컬만 수정하고 git/GAS 반영 안 하면 버전 불일치 발생!
...
- 라인업 테이블에서 **티어/유형 컬럼 숨김** (민감정보 보호, 직업은 표시)
- 라인업 테이블에 **전화번호 컬럼 추가** (클릭시 바로 전화 연결)
