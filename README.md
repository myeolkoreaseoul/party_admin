...
### ✅ 2026-01-09 최신 수정 (고질적 버그 해결)

**1. 고객정보 표시 버그 해결 (이상윤 문제)**
- **원인**: snake_case(`birth_year`) vs camelCase(`birthYear`) 필드명 불일치
- **해결**: `getCustomerField()` 유틸 함수 추가 → 자동으로 둘 다 체크
- **효과**: 수기 입력한 고객 정보가 예약관리/라인업에 정상 표시

**2. 조사 완료 판단 로직 통일**
- **원인**: 예약관리는 survey 테이블만 체크, 고객관리는 customer 정보도 체크
- **해결**: `hasCustomerInfo()` 함수로 통일 → customer에 성별+출생연도 있으면 "완료"
- **효과**: 수기 입력 고객도 조사 "완료"로 표시

**3. 블랙리스트 필터링 개선**
- **원인**: `is_blacklisted` 필드만 체크, 메모는 무시
